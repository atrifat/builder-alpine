...
RUN apk add --no-cache bash curl wget musl-dev build-base micro htop binutils coreutils git cmake meson bison ninja \
...
RUN apk add --no-cache bzip2-dev curl-dev icu-dev libgcrypt-dev libjpeg-turbo-dev libpq-dev libpng-dev libsodium-dev libwebp-dev \
    ...
    libevent-dev c-ares-dev pcre2-dev re2c fuse-dev libnfs-dev libffi-dev\
